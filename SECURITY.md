...
- **컨트롤러 수준 권한 검증**: Spring Security는 `anyRequest().permitAll()` 구성이며, 인증/인가 검증은 컨트롤러에서 직접 수행 (향후 정책 중심 구조로 전환 예정)
...
- **CSP**: Content-Security-Policy 적용 (현재 `unsafe-inline` 포함되어 있어 향후 정책 강화 여지 있음)
...
- **Content-Security-Policy**: 인라인 스크립트 및 외부 리소스 제한 (Turnstile 도메인 `challenges.cloudflare.com` 허용)
- **HSTS**: 1년 유지, 서브도메인 포함
- **Referrer-Policy**: `strict-origin-when-cross-origin`
- **외부 리소스**: Spline 3D Viewer CDN (`unpkg.com`) 사용 시 CSP 및 Subresource Integrity 검토 필요

### 봇 방지 및 CAPTCHA

- **Cloudflare Turnstile**: 로그인 및 회원가입 폼에 적용
  - 사이트 키 기반 클라이언트 렌더링
  - 서버 측 `siteverify` API 검증 (`TurnstileService`)
  - 토큰 길이 검증 (최대 2048자)
  - 클라이언트 IP 연동 검증
...
- [ ] `DB_PASSWORD` 환경 변수가 설정되었는가? (폭백 기본값 없음)
- [ ] H2 콘솔이 비활성화되었는가?
- [ ] SQL 로그가 비활성화되었는가?
- [ ] HTTPS가 적용되었는가?
...
- [ ] 외부 CDN(Spline, Turnstile) 리소스에 대한 CSP 및 SRI 정책을 검토했는가?
...
- **[HIGH]** Cloudflare Turnstile 도입 — 로그인/회원가입에 봇 방지 및 자동화 공격 차단 적용
  - 클라이언트: `cf-turnstile` 위젯 렌더링 (`challenges.cloudflare.com`)
  - 서버: `TurnstileService`에서 `siteverify` API 호출 및 토큰 검증
  - CSP: `script-src`, `connect-src`, `frame-src`에 `challenges.cloudflare.com` 추가
  - 환경 변수: `TURNSTILE_SECRET_KEY`로 비밀 키 관리
- **[MEDIUM]** Spline 3D Viewer 추가 — 외부 CDN(`unpkg.com`) 리소스 사용으로 CSP 정책 및 SRI 검토 필요
- **[MEDIUM]** 모바일 환경 애니메이션 분리 — 모바일 기기에서 GSAP 애니메이션 로직 분리로 성능 및 보안 안정성 향상
- **[LOW]** CSS overflow 속성 개선 — `overflow-x: hidden` 및 스크롤 동작 개선으로 잠재적 UI 레이아웃 취약점 제거
- **[LOW]** 업로드 파일 캐시 제어 강화 — 정적 리소스 핸들러 설정 개선으로 업로드 파일 캐싱 정책 명확화

### 2026-03-11 (v3)

- **[HIGH]** 회원 탈퇴 기능 구현 — 비밀번호 재확인, 연관 데이터 순서 삭제 (Lh → Comment → Post cascade → User), 프로필 이미지 파일 삭제
- **[HIGH]** 회원 탈퇴 Path Traversal 방지 — canonical path 검증으로 `upload-dir` 외부 경로 차단
- **[HIGH]** 회원 탈퇴 브루트포스 방어 — `LoginAttemptService` 재사용, 5회 실패 시 15분 잠금
- **[MEDIUM]** BCrypt DoS 방지 — 비밀번호 입력 100자 초과 시 즉시 거부
- **[MEDIUM]** 게시글 수정 시 이미지 관리 구현 — 기존 이미지 선택 삭제 및 신규 이미지 추가, 동일한 파일 업로드 보안 검증 적용
- **[LOW]** 정적 리소스 브라우저 캐싱 — `Cache-Control: public, max-age=2592000` (30일) 설정으로 CSS/JS/이미지 반복 다운로드 방지
