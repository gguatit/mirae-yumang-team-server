...
- **GitHub/Google OAuth2 소셜 로그인**: Spring Security OAuth2 Client, 계정 연동 지원
...
- **SameSite Cookie**: Lax 정책으로 CSRF 추가 방어 (Strict일 경우 OAuth cross-site redirect 실패)
...
### 2026-07-14 (v7)
- Google OAuth2 소셜 로그인 추가
- SameSite 쿠키 정책 Strict → Lax 변경 (OAuth2 cross-site redirect 대응)
- OAuth2 성공 핸들러 provider별 속성 추출 분기 처리 (Google: sub/name, GitHub: id/login)
