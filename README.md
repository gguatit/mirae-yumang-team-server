...
미래유망팀의 Spring Boot 기반 통합 커뮤니티 플랫폼입니다. 게시판, AI 챗봇 등 다양한 기능을 제공합니다.
...
- **게시판**: 게시글 작성, 수정, 삭제, 조회수, 좋아요/싫어요, 검색 및 정렬
  - **실시간 업데이트**: 5초 폴링으로 새 게시글 자동 표시 (새로고침 불필요)
  - **초성 검색**: es-hangul 라이브러리 기반 한글 초성 검색 지원 (예: "ㅅㄱ" 검색)
- **댓글 시스템**: 게시글별 댓글 작성 및 관리
- **파일 업로드**: 이미지 업로드 및 저장 (최대 10MB)
- **AI 챗봇**: 챗봇 인터페이스
- **마이페이지**: 사용자 프로필 및 작성글 관리
- **홈페이지**: 
  - GSAP 기반 부드러운 스크롤 애니메이션
  - 그라데이션 스크롤 진행도 표시
  - 반응형 디자인 (1200px, 768px 브레이크포인트)
...
| **Frontend Libraries** | GSAP 3.12.5, ScrollTrigger, es-hangul 2.3.8 |
...
| GET | `/posts/api/new?since={datetime}` | 실시간 새 게시글 조회 (폴링용) |
